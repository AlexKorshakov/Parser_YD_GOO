        ...
        try:
            ...
            if request.status_code == 200:  # если запрос был выполнен успешно то
                ...
                if len(divs) > 0:  # если ответ на запрос что то содержит то
                    ...
                    divs_text_shelves(divs, it_url['ques'], divs_requests)  # парсим данные ответа
                    url_counter += 1
                else:
...
def divs_text_shelves(my_divs, url_ques, my_divs_requests=None):
        ...
        try:
            ...
            try:
                ...
                for divs_iter in my_divs_requests:  # записываем данные
                    ...
                    wb.Worksheets('Лист1').Cells(doc_row, 1).Value = divs_iter['ques']
                    wb.Worksheets('Лист1').Cells(doc_row, 2).Value = divs_iter['company_cid']
                    wb.Worksheets('Лист1').Cells(doc_row, 3).Value = divs_iter['company_link_1']
                    wb.Worksheets('Лист1').Cells(doc_row, 4).Value = divs_iter['company_title']
                    wb.Worksheets('Лист1').Cells(doc_row, 5).Value = divs_iter['company_sitelinks']
                    wb.Worksheets('Лист1').Cells(doc_row, 6).Value = divs_iter['company_text']
                    wb.Worksheets('Лист1').Cells(doc_row, 7).Value = divs_iter['company_contact']
                    doc_row += 1
...
def main():
    ...
    full_path = r'C:\Users\DeusEx\PycharmProjects\Parser_YD_GOO\Parser_Yandex.xlsx'
